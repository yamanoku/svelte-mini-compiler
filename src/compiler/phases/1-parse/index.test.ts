...
test("引用符なし属性値の直後の /> は自己終了として扱う", () => {
  const ast = parse("<div><span id=x/>after</div>");

  const div = ast.fragment.nodes[0] as RegularElement;
  const span = div.fragment.nodes[0] as RegularElement;
  assert.equal(span.name, "span");
  assert.deepEqual(
    span.attributes.map(({ name, value }) => ({ name, value })),
    [{ name: "id", value: "x" }],
  );
  // 自己終了なので "after" は span の子ではなく div の子になる
  assert.equal(span.fragment.nodes.length, 0);
  assert.equal((div.fragment.nodes[1] as Text).data, "after");
});

test("引用符なし属性値は / を含められる", () => {
  const ast = parse("<a href=/foo/bar>x</a>");

  const a = ast.fragment.nodes[0] as RegularElement;
  assert.deepEqual(
    a.attributes.map(({ name, value }) => ({ name, value })),
    [{ name: "href", value: "/foo/bar" }],
  );
});
...
test("デコードできない数値文字参照はクラッシュせず原文のまま残る", () => {
  // &#9999999; は 0x10FFFF を超え、&#xZZ; は数値として解釈できない
  const ast = parse("<p>&#9999999; と &#xZZ;</p>");

  const p = ast.fragment.nodes[0] as RegularElement;
  const text = p.fragment.nodes[0] as Text;
  assert.equal(text.data, "&#9999999; と &#xZZ;");
});
...
test("raw-text要素の終了タグは大文字小文字を区別しない", () => {
  const ast = parse("<script>let a = 1;</SCRIPT>");

  assert.ok(ast.instance);
  assert.equal(ast.instance.content, "let a = 1;");

  const div_ast = parse("<div><style>p {}</STYLE></div>");
  const div = div_ast.fragment.nodes[0] as RegularElement;
  const style = div.fragment.nodes[0] as RegularElement;
  assert.equal(style.name, "style");
  assert.equal((style.fragment.nodes[0] as Text).raw, "p {}");
});

test("void要素の明示的な終了タグは専用のエラーになる", () => {
  assert.throws(
    () => parse("<div><br></br></div>"),
    (error: unknown) => {
      assert.ok(error instanceof CompileError);
      assert.match(error.message, /void要素/);
      return true;
    },
  );
});
