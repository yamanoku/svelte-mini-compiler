...
test("インライン要素間のスペースは単一スペースのテキストノードとして出力される", () => {
  const result = compile("<b>a</b> <i>b</i>");
  assert.match(result.js.code, /createTextNode\(" "\)/);
});

test("改行を含む空白のみのテキスト（要素間のインデント）は出力されない", () => {
  const result = compile("<div>\n\t<p>hi</p>\n</div>");
  assert.doesNotMatch(result.js.code, /createTextNode\("\\n/);
});
